...
export function checkSpendingLimit(
  ...
  const spendingLimit = getCustomerRoleSpendingLimit(customer.employee);
...
export function getCustomerRoleSpendingLimit(employee: QueryEmployee | null) {
  if (!employee) {
    return 0;
  }

  const company = employee.company as QueryCompany | undefined;
  const role = employee.role || (employee.is_admin ? "company_admin" : "buyer");
  const roleLimits = (company?.role_spending_limits || {}) as Record<
    string,
    unknown
  >;
  const roleLimit = Number(roleLimits[role] || 0);
